/*
 * deactivation of the exp for the moment, awaiting modularity
 ...
 * ConsoleManager.create("An error occured while adding xp to " +
 * user.getGlobalName()+"Please check").error().withType(EnumLogType.NETWORK).
 * displayOnDiscord().end();
 * }
 * }
 * 
 * /**
 ...
 * final String userName = user.getGlobalName() == null ? user.getName() :
 * user.getGlobalName();
 * try {
 ...
 * 
 * try {
 * final User user = event.getOption("user") == null ? event.getUser() :
 * event.getOption("user").getAsUser();
 * final String userName = user.getGlobalName() == null ? user.getName() :
 * user.getGlobalName();
 * final String userAvatarUrl = user.getAvatarUrl() == null ?
 * user.getDefaultAvatarUrl() : user.getAvatarUrl();
 ...
 * 
 * BufferedImage image = new BufferedImage(widthPicture, heightPicture,
 * BufferedImage.TYPE_INT_ARGB);
 * Graphics2D g2d = image.createGraphics();
 * g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
 * RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
 * PhotonInterfaceUtils.drawRoundedRect(g2d, 0, 0, widthPicture, heightPicture,
 * 40, 40, new Color(24, 26, 28));
 * 
 ...
 * } catch (IOException e) {
 * e.printStackTrace();
 * event.reply("An error occured while getting the level of " +
 * event.getUser().getGlobalName() + "Please retry later. (Staff is
 * notified)").queue();
 * } catch (SQLException e) {
 * event.reply("An error occured while getting the level of " +
 * event.getUser().getGlobalName() + "Please retry later. (Staff is
 ...
 * "+TranslationManager.format(UsersInfo.getLanguage(user.getId()).code,"discord.levelMessage.title")+"
 * <span style='color: #8b2628'>"+name+"</span>
 * </p>
 * </html>";
 ...
 * }
 * 
 * private static void drawAvatar(Graphics2D g2d, String url, int x, int y, int
 * width, int height) {
 * try {
 * BufferedImage avatar = ImageIO.read(new URL(url));
 * PhotonInterfaceUtils.drawRoundedImage(g2d, avatar, x, y, width, height, 200,
 * 200);
 * } catch (IOException e) {
 * e.printStackTrace();
 * }
 ...
 */
/*
 * private static JPanel getProgressBar(int Userxp, int XpLevel, int
 * levelNumber, User user){
 ...
 * JPanel panel = new JPanel();
 * 
 * ColoredProgressbar progressBar = new ColoredProgressbar(new Color(100, 100,
 * 100), new Color(139, 38, 40));
 ...
 * 
 * String levelString =
 * "<html><p style='color: white'>"+TranslationManager.format(UsersInfo.
 * getLanguage(user.getId()).code,"discord.levelMessage.description")
 * +" <span style='color: #8b2628'>"+levelNumber+"</span></p></html>";
 ...
 * 
 * String xpString = String.
 * format("<html><p style='color: #A4A4A4'>%s <span style='color: white'>/</span> <span style='color: #8b2628'>%s</span></p></html>"
 * , Userxp, XpLevel);
