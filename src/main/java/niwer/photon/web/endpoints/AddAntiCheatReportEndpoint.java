...
public class AddAntiCheatReportEndpoint implements IEndpoint {
    ...
    public void handle(Context handler) {
        ...
        if (FILE_MESSAGE == null || USER_UUID == null || OPERATRING_SYSTEM == null) {
            handler.status(400).result("Missing parameters");
            return;
        }

        /* Ensure parameters are not blank */
        if (FILE_MESSAGE.isBlank() || USER_UUID.isBlank() || OPERATRING_SYSTEM.isBlank()) {
            handler.status(400).result("Parameters cannot be blank");
            return;
        }

        AnticheatTable.save(USER_UUID, FILE_MESSAGE, OPERATRING_SYSTEM);
